...
setproctitle('yerim_Experiments_5-2')
...
if __name__ == '__main__':
    ...
    print("Experiments_5-2_seed_2020")
    ...
    """build dataset"""
    train_cf, test_cf, user_dict, train_kg_dict, n_params, graph_AK, graph_UIS, mat_list = load_data(args, device)
    ...
    model = Recommender(n_params, args, graph_AK, mean_mat_list[0],seed).to(device)
    ...
    for epoch in range(args.epoch):
        ...
        while s + args.batch_size <= len(train_cf):
            ...
            batch_loss, _, _, batch_cor = model(batch, graph_AK, graph_UIS)
            ...
            #cor_loss += batch_cor
            #cor_loss+=float(batch_cor)
            cor_loss+=batch_cor.item()
        ...
        if epoch % 10 == 9 or epoch == 1:
            """testing"""
            test_s_t = time()
            ret = test(model, graph_AK, graph_UIS, user_dict, n_params)
            test_e_t = time()
           ...
           # torch.cuda.empty_cache()
            ...
            print('using time %.4f, training loss at epoch %d: %.4f, cor: %.6f' % (train_e_t - train_s_t, epoch, loss, cor_loss))
